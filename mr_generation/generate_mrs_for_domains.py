...
class MRGenerator:
    def __init__(self, domain, num_variations, num_samples=None):
        ...
        print("slots initialized: ", self.slots)
        ...
        print("val constraints: ", self.value_constraints)


    def create_mrs_from_movie_data(self, file_in, da=None, num_slots=None, max_num_slots=None):
        ...
        # Read in the CSV file with the movie data
        ...
        for i, row in df.iterrows():
            ...
            mr_var_cnt = 0
            num_loops = 0
            num_slot_cnt = num_slots
            print(num_slot_cnt, max_num_slots)
            while num_slot_cnt <= max_num_slots:
                mr_var_cnt = 0
                while mr_var_cnt < self.num_variations:
                    # Generate a random MR from the current game's data
                    print("row: ", row)
                    mr_dict = self.__get_random_slot_comb_for_da(row, da, num_slot_cnt)
        ...
        with open('tv_actors_mrs_new.csv', 'a', encoding='utf8') as fi_out:
            for mr in mrs:
                fi_out.write(da + "\n")
        
        # set your output file name here
        file_out = "tv_actors_mrs_new_test.txt"
        with open(file_out, 'a', encoding='utf8') as f_out:
            print("mrs to write: ", mrs)
            f_out.write('\n'.join(mrs))
...
#        else:
        ...
        print("slot_val_dict: ", slot_value_dict)
        print("mandatory slots: ", mandatory_slots)
        print("excluded_slots: ", excluded_slots)
        ...
        if num_slots is not None:
            ...
            print("da, additional slots: ", da, num_additional_slots)
            if num_additional_slots > 0:
                print("slot items: ", slot_value_dict.items())
                if len(slot_value_dict.items()) >= num_additional_slots:
                    mr_dict.update(random.sample(slot_value_dict.items(), num_additional_slots))
                else:
                    mr_dict.update(slot_value_dict.items())
                #print("mr dict updated: ", mr_dict)
        else:
            # Add all the remaining non-excluded slots
            mr_dict.update(slot_value_dict)

        #print("slot val dict update: ", slot_value_dict)
        self.__adjust_mr_for_da(da, mr_dict)

        #print("mr_dict: ", mr_dict)
    ...
    def __adjust_mr_for_da(self, da, mr_dict):
        ...
        if da == 'request':
            ...
            mr_dict['specifier'] = random.sample(specifiers, 1)[0] 
...
def main():
    ...
    # add constraints for slot range
    slot_range_constraints = {
        "inform": [3, 8],
        "confirm": [2, 3],
        "give_opinion": [3, 4],
        "recommend": [2, 3],
        "request": [1, 2],
        "request_attribute": [1],
        "request_explanation": [2, 3],
        "suggest": [2, 3],
        "verify_attribute": [3, 4]
    }
    ...
    num_variations = 5
    num_total_samples = None
    da = 'request_attribute'
    #if len(slot_range_constraints[da]) == 1:
    #   num_variations = 1
    #else:
    #    num_variations = \
    #        (slot_range_constraints[da][-1] - slot_range_constraints[da][0])
    num_slots = slot_range_constraints[da][0]
    #num_slots = None
    max_num_slots = slot_range_constraints[da][-1]

    mr_gen = MRGenerator(domain, num_variations, num_total_samples)

    # ----

    #games_file_in = os.path.join(VIDEO_GAME_DATA_DIR, 'generation', 'video_games.csv')
    movie_file_in = os.path.join('movies_data.csv')

    #mr_gen.create_mrs_from_game_data(games_file_in, da, num_slots)
    for da in slot_range_constraints.keys():
        max_num_slots = slot_range_constraints[da][-1]
        mr_gen.create_mrs_from_movie_data(movie_file_in, da, num_slots, max_num_slots)
